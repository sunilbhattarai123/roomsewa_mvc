...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["sendOTP"])) {
        ...
        if ($emailCount == 1) {
            $myquery = "SELECT owner_id as id from owner where email='$email'";
            ...
            $owner_id = $row1['id'];
                ...
                try {
                    ...
                    $sql = "UPDATE owner SET resetOtp='$hashedOtp' where owner_id='$owner_id'";
                    mysqli_query($db, $sql);
        ...
        $sql = "SELECT resetOtp as otp from owner where email='$email'";
        ...
        $sql = "UPDATE owner SET password = '$newPassword', resetOtp = NULL WHERE email = '$email'";
        mysqli_query($db, $sql);

        echo '<div style="margin-top: 20px;" class="alert alert-success" role="alert">
                 Password reset successfully. You can now log in with your new password.
             </div>';
        echo '<script>window.location.href = "owner-login.php";</script>';
    }
}
?>
    ...
    <div class="container reset-container">
        ...
        if ($step == 1) {
            ...
            <form method="post">
                ...
                <button type="submit" name="sendOTP" class="btn btn-primary btn-block" style="background-color:#5cb85c;>Send OTP</button>
            </form>
            <?php
        } elseif ($step == 2) {
            // Step 2: Enter OTP
